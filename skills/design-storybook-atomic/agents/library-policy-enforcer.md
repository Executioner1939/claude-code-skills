...
description: >
  ...
  canonical fix per defect. Read-only. Used by /design-storybook-atomic:audit-libraries
  and as a cross-cutting pass in audit-atomic / audit-molecules / audit-organisms.
...
- `exemptions` — optional, parsed from `.design-storybook-atomic.yml`.
...
DEPENDENCY AUDIT (package.json / lockfile)
  ...
  Forbidden (in dependencies — block merge):
    [F1] react-hook-form  ^7.54.0  (path: package.json:34)
         ...
         exemption: none (no entry in .design-storybook-atomic.yml)
...
EXEMPTIONS HONORED
  (none active in .design-storybook-atomic.yml)
...
- `.design-storybook-atomic.yml` if present — `library_policy.exemptions[]`.
