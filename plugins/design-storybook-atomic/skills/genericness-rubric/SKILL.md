...
The `atomic-auditor` agent emits this block per component, slotted between the existing HYGIENE and COMPOSITE blocks. Format mirrors the existing axes (see `plugins/design-storybook-atomic/agents/atomic-auditor.md` lines 54–101) so downstream parsers do not change shape:
