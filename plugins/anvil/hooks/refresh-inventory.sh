...
#        - $CLAUDE_PROJECT_DIR/.anvil/ already exists, or
#        - $CLAUDE_PROJECT_DIR/.anvil.yml exists
...
# .anvil/inventory.json. Errors are logged to
# .anvil/scan.log and silently swallowed — a stale
...
# Opt-in gate.
INV_DIR="$PROJECT_DIR/.anvil"
if [[ ! -d "$INV_DIR" && ! -f "$PROJECT_DIR/.anvil.yml" ]]; then
  exit 0
fi
