...
Read `atomic-design`, `component-composition`, `tanstack-integration`, `approved-libraries` (auto-preloaded into chained subagents). Read `<scope>/.anvil.yml` for any pre-deferred clusters (`merge.deferred[]`).

## Step 1 — Cartography

Spawn `component-cartographer`. HANDOFF: `<scope>/.anvil/handoffs/merge-duplicates-<run>/phase-01-cartography-to-dedup.md`.
...
HANDOFF: `<scope>/.anvil/handoffs/merge-duplicates-<run>/phase-02-clusters-to-analysis.md`.
...
Each cluster gets its own HANDOFF: `<scope>/.anvil/handoffs/merge-duplicates-<run>/phase-03-cluster-<n>-analysis.md`.
...
2. **Defer** — skip, log to `<scope>/.anvil.yml` `merge.deferred[]` with reasoning.
...
HANDOFF: `<scope>/.anvil/handoffs/merge-duplicates-<run>/phase-05a-merge-cluster-<n>.md`.
...
HANDOFF: `<scope>/.anvil/handoffs/merge-duplicates-<run>/phase-05b-codemod-cluster-<n>.md`.
...
HANDOFF: `<scope>/.anvil/handoffs/merge-duplicates-<run>/phase-05c-stories-cluster-<n>.md`.
...
HANDOFF: `<scope>/.anvil/handoffs/merge-duplicates-<run>/phase-05d-verify-cluster-<n>.md`.
...
DEFERRED CLUSTERS (logged to .anvil.yml):
...
- **Theming divergence.** Two members because two themes need different defaults. Solution is per-theme tokens, not duplicate components — recommend `/anvil:audit-tokens` first.
