...
Read `story-coverage-checklist` (rubric), `tanstack-integration` (contract), `approved-libraries` (policy), `<scope>/.anvil.yml`. Check baseline at `<scope>/.anvil/baseline-coverage.md`.

## Step 1 — Inventory

Spawn `component-cartographer`. HANDOFF: `<scope>/.anvil/handoffs/coverage-report-<run>/phase-01-cartography-to-analysts.md`.
...
HANDOFF: `<scope>/.anvil/handoffs/coverage-report-<run>/phase-02-analysts-to-orchestrator.md`.
...
HANDOFF: `<scope>/.anvil/handoffs/coverage-report-<run>/phase-03-policy-to-orchestrator.md`.
...
SECTION 7 — TOP RISKS
  ...
  2. 12 hygiene fails — 9 are hardcoded color literals. → /anvil:audit-tokens
  3. 4 candidate duplicate clusters detected. → /anvil:merge-duplicates
  4. 3 organisms call useNavigate() — domain coupling violation.
  5. Library-policy compliance: 71/100 (BLOCKED). → /anvil:audit-libraries
...
NEXT
  - /anvil:audit-organisms (deep-dive blocked organisms)
  - /anvil:audit-tokens (clear hygiene fails)
  - /anvil:audit-libraries (clear policy gaps)
  - /anvil:merge-duplicates (consolidate 4 clusters)
...
- Baseline (replaces previous): `<scope>/.anvil/baseline-coverage.md`
- Dated history: `<scope>/.anvil/history/coverage-<YYYY-MM-DD>.md`
...
2. **Reproducibility.** Surface `.anvil.yml` overrides at the top of the report.
...
- **Stale `.anvil.yml`** (override references a level that no longer exists). Warn but proceed with defaults for unknown keys.
