...
Read `<scope>/.anvil.yml` if present. Honor:
...
```bash
test -f "$scope_path/.anvil/baseline-libraries.md" \
...
```text
<scope>/.anvil/handoffs/audit-libraries-<run>/phase-01-cartographer-to-policy.md
<scope>/.anvil/handoffs/audit-libraries-<run>/phase-02-policy-to-orchestrator.md
```
...
1. **Baseline (replaces previous)** — `<scope>/.anvil/baseline-libraries.md`
2. **Dated history (additive)** — `<scope>/.anvil/history/libraries-<YYYY-MM-DD>.md`
...
3. **HONOR `.anvil.yml`** — exemptions with valid sunsets are not defects.
...
- **`.anvil.yml` malformed.** Warn loudly, parse what's parseable, treat unknown keys as ignored, NEVER apply policies that aren't in the explicit policy doc.
