...
These are enforced by `audit-atoms`, `audit-molecules`, `audit-organisms`, and `audit-libraries`. Bypassing the listed integration = hygiene fail.
...
- **`genericness-rubric`** — the formal test for whether a component is a generic primitive vs. a misplaced domain instance. Used by `audit-atoms`, `audit-molecules`, `audit-organisms`.
