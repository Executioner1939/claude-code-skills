...
This is not a SKILL.md and does not auto-load. It's referenced by the `storybook-authoring`, `storybook-configuration`, `audit-atoms`, `audit-molecules`, `audit-organisms`, and `add-component` skills when a project is detected on a legacy version.
...
If a particular story still fails after migration, the audit workflows in this plugin (`/anvil:audit-atoms` etc.) will surface the per-component breakdown.
...
If `audit-atoms` (or the molecule / organism variants) detect a project still on CSF2 or CSF3, they:
