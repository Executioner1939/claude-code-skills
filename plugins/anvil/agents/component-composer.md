...
description: >
  ...
  the heart of /anvil:add-component, and on demand for
...
2. **Existence pre-check (MANDATORY before any verdict).** Before any scoring, query the inventory for an existing implementation of the spec's name at any tier:
   ...
   python3 "${CLAUDE_PLUGIN_ROOT}/scripts/inventory.py" \
     query "$scope/.anvil/inventory.json" \
     contract <name>
   ```
   ...
   Stop. Do not proceed to verdict scoring unless the user explicitly overrides with `--allow-tier-duplication`. The audit-atoms feedback called out this exact failure: a previous session built `Card.tsx` and `Toast.tsx` atoms before the orchestrator's redirect arrived, wasting the work because both already existed at higher tiers.
...
NEXT STEP
  (if REUSE) — stop here.
  (if EXTEND) — call /anvil:add-component again with mode=extend, target=<path>.
...
6. **Stop on missing tokens.** If implementation needs a token that doesn't exist, stop. The user must add the token first (run `/anvil:audit-tokens`).
