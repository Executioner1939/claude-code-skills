...
If the project gates motion globally (a `@media (prefers-reduced-motion: reduce)` block in `tokens.css`, OR Tailwind's `motion-safe:` defaults are wired), record `MOTION_GLOBAL=true` and SKIP the per-component motion-guard check. Flagging it produces false positives — the audit-atoms feedback called out this case explicitly.
