...
- **Auto-refresh PostToolUse hook** at `hooks/refresh-inventory.sh`. After every Edit / Write / MultiEdit on a component file, inventory rebuilds in the background (30s debounce; opt-in via the project owning a `.anvil/` directory).
- **Phase 0 in `audit-atoms`** — rubric detection (MDX convention, form contract, tier placement, token convention, lint rules affecting atoms, motion-reduce defaults), baseline integrity probe quarantines pre-existing breakage, tier-aware contract resolution, mode flags (`--auto-baseline=replace|history-only|prompt`, `--static-only`, `--no-prompt`), cardinality cutoff (> 30 atoms ⇒ static-scan grading).
...
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/inventory.py \
  scan --root . --tier all --out .anvil/inventory.json

# Common queries
inventory.py query .anvil/inventory.json stats
inventory.py query .anvil/inventory.json consumers-of atoms/Button --transitive
inventory.py query .anvil/inventory.json reconciliation
inventory.py query .anvil/inventory.json contract toast      # find Toast across tiers
inventory.py query .anvil/inventory.json centrality          # requires `pip install networkx`

# Visualize
inventory.py export .anvil/inventory.json --format mermaid > graph.mmd
inventory.py export .anvil/inventory.json --format gexf    > graph.gexf  # Gephi
```
...
- **Inter-agent HANDOFF.md contract.** Every multi-agent workflow now writes a HANDOFF.md per phase boundary at `<scope>/.anvil/handoffs/<workflow>-<run>/`; orchestrator halts if any subagent skips printing `HANDOFF: <path>`.
- **Audit-history (mirrors `terraform-audit`).** Tier-1 baseline diff at `<scope>/.anvil/baseline-<level>.md`; Tier-2 dated history at `<scope>/.anvil/history/<level>-<date>.md`. Every audit re-renders Section 9 — Diff vs baseline on subsequent runs.
...
|---|---|
| `/anvil:audit-atoms` | Grade every atom: coverage, quality, hygiene (TanStack contract + tokens + library policy + a11y). Baseline diff. |
| `/anvil:audit-molecules` | Same, scoped to molecules. Adds composition correctness, no-domain-state-leak, FormField-must-accept-field. |
