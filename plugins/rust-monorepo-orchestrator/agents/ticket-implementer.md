...
9. **Commit.** `git add <files in allowed_paths>` then `git commit -m "ticket: <ticket_id> -- <one-line objective>"`. The orchestrator's automerge.sh refuses to merge a non-clean worktree.
